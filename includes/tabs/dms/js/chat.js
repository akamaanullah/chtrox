    ...
    function initChat() {
            ...
            function sanitizeNode(node) {
                ...
                if (node.nodeType === 1) {
                    ...
                    for (var i = node.attributes.length - 1; i >= 0; i--) {
                        ...
                        if (name !== 'style' && name !== 'class' && name !== 'align') node.removeAttribute(name);
                    }
                }
            }
            sanitizeNode(el);
        ...
        if (reactionPicker && dmChatMessages) {
            function positionReactionPicker(btn) {
                ...
                var pickerW = reactionPicker.offsetWidth;
                var pickerH = reactionPicker.offsetHeight;
                var gap = 8;
                /* Action bar ke neeche picker dikhao (message → action bar → reactions) */
                var top = rect.bottom + gap;
                var left = rect.left + (rect.width / 2) - (pickerW / 2);
                if (top + pickerH > window.innerHeight - gap) {
            ...
            function closeReactionPicker() {
                ...
                if (currentReactMessage) {
                    currentReactMessage.classList.remove('dm-chat-msg--picker-open');
                }
