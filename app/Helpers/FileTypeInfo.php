...
{
    /**
     * @return array{extLabel: string, extSlug: string, typeLabel: string, lucideIcon: string, iconCategory: string}
     */
    public static function get(string $name, string $mimeType = ''): array
    {
        $parts = explode('.', $name !== '' ? $name : 'File');
        $extRaw = count($parts) > 1 ? (string) array_pop($parts) : '';
        $extSlug = strtolower(preg_replace('/[^a-z0-9]/', '', $extRaw) ?? '') ?: 'file';

        if ($extSlug === 'file' && $mimeType !== '') {
            if (str_contains($mimeType, 'pdf')) {
                $extSlug = 'pdf';
            } elseif (str_contains($mimeType, 'spreadsheet') || str_contains($mimeType, 'excel') || $mimeType === 'text/csv') {
                $extSlug = 'csv';
                ...
                $extSlug = 'txt';
            } elseif (str_contains($mimeType, 'font')) {
                $extSlug = 'ttf';
            } elseif ($mimeType === 'application/json') {
                $extSlug = 'json';
            } elseif ($mimeType === 'application/xml' || $mimeType === 'text/xml') {
                $extSlug = 'xml';
            } elseif ($mimeType === 'application/sql') {
                $extSlug = 'sql';
            } elseif (str_contains($mimeType, 'presentation')) {
                $extSlug = 'pptx';
            }
        }
        ...
        if (isset($iconMap[$extSlug])) {
            $icon = $iconMap[$extSlug];
            if ($icon === 'file-spreadsheet' || $extSlug === 'csv' || $extSlug === 'tsv' || $extSlug === 'numbers') {
                $iconCategory = 'spreadsheet';
            } elseif ($icon === 'presentation') {
                $iconCategory = 'presentation';
            } elseif ($icon === 'image' || $icon === 'layers' || $icon === 'pen-tool') {
                $iconCategory = 'image';
            } elseif ($icon === 'video') {
                $iconCategory = 'video';
            } elseif ($icon === 'music' || $icon === 'music-2') {
                $iconCategory = 'audio';
            } elseif ($icon === 'archive' || $icon === 'disc') {
                $iconCategory = 'archive';
            } elseif ($icon === 'file-code' || $icon === 'terminal' || $icon === 'container' || $icon === 'cpu') {
                $iconCategory = 'code';
            } elseif ($icon === 'database' || $icon === 'file-braces' || $icon === 'settings' || $icon === 'settings-2' || $icon === 'share-2') {
                $iconCategory = 'data';
            } elseif ($icon === 'type') {
                $iconCategory = 'font';
            } elseif ($icon === 'package' || $icon === 'smartphone' || $icon === 'library' || $icon === 'binary') {
                $iconCategory = 'package';
            } elseif ($extSlug === 'pdf') {
                $iconCategory = 'pdf';
            } elseif ($icon === 'file-text' || $icon === 'book-open') {
                $iconCategory = 'document';
            }
        }

        return [
            'extLabel' => strtoupper($extRaw !== '' ? $extRaw : $extSlug),
            'extSlug' => $extSlug,
